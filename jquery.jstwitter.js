$(function() {
	...
	JQTWEET = {
	    ...
	    user: 'lnao_im', //username
	    numTweets: 801, //number of tweets
	    appendTo: '#jstwitter',
	    ...
	    loadTweets: function(hashtag, appendTo) {
                if (appendTo) {
                   JQTWEET.appendTo = appendTo; }
                else JQTWEET.appendTo = '#jstwitter';
                console.log(JQTWEET.appendTo);
                         ...
                         if (typeof data === 'string' ) {
                             ...
                             if (JQTWEET.search) data = data.statuses;
                                   ...
                                   if ($.inArray(hashtag, hashtags) != -1 || hashtag === ''){
                                           ...
                                           try {
                                             if (data[i].entities['media']) {
                                               img = '<a href="' + data[i].entities['media'][0].media_url
                                                  + '" target="_blank"><div class="pic" style="overflow:hidden; max-height:300px"><img style="margin-top:-'+
                                                   (data[i].entities['media'][0].sizes.medium.h - 300)*1.0/2.0 + 'px" src="'
                                                  + data[i].entities['media'][0].media_url + '" /></div></a>';
                                             }
                                           } catch (e) {
                                             //no media
                                           }
                              ...
                              if ($("div.item").length == 0)
                                  $(JQTWEET.appendTo).append( JQTWEET.template.replace('{TEXT}', JQTWEET.ify.clean("No tweets found with this hashtag.") )
                                      .replace('{USER}', "Machine")
                                      .replace('{IMG}', "" )
                                      .replace('{AGO}', "now" )
                                      .replace('{LOC}', " from here " )
                                      );
